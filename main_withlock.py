...
class promfsd:
    ...
    def __init__(self, filepath, pj, rtargets, rlabels):
        ...
        self.file_data = self.getfiledata(self.file_path)
        self.pj = pj
    ...
    def getfiledata(filepath):
        while True:
            try:
                if os.path.isfile('%s.lock' % filepath):
                    continue
                else:
                    open('%s.lock' % filepath, 'a').close()
                    with open(filepath) as f:
                        return json.load(f)
            except Exception:
                raise ImportError('JSON file in wrong format')
    ...
    def processfile(self):
        if not (self.rtargets or self.rlabels):
            ...
            self.removelabels()
        os.remove('%s.lock' % self.file_path)

    def write_to_disk(self):
        f = open('%s.new' % self.file_path , 'w+')
